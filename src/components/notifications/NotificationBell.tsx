...
export function NotificationBell() {
  ...
  const [activeTab, setActiveTab] = useState<'AGREEMENT' | 'LOCK_IN'>('AGREEMENT');
  
  const [summary, setSummary] = useState<NotificationSummary>({ agreementCount: 0, lockinCount: 0, totalCount: 0 });
  ...
  // Fetch agreement & lock-in notifications
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.success) {
          setSummary(json.summary || { agreementCount: 0, lockinCount: 0, totalCount: 0 });
          setAgreements(json.agreements || []);
          setLockins(json.lockins || []);

          // Default to tab that has notifications
          if ((json.summary?.agreementCount || 0) === 0 && (json.summary?.lockinCount || 0) > 0) {
  ...
  const activeList = activeTab === 'AGREEMENT' ? agreements : lockins;
    ...
    <div className="relative" ref={popoverRef}>
      ...
      >
        <button
          ...
          title="Renewal & Lock-in Notifications"
          aria-label="Renewal & Lock-in Notifications"
        >
        ...
        {/* Hover Summary Card (Visible when hovering Bell icon and popover is closed) */}
        <AnimatePresence>
          {isHoveredBell && !isOpen && (
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 w-64 bg-[#1B1C1C] text-white p-3 shadow-xl z-50 border border-neutral-700 pointer-events-none text-xs"
            >
              <div className="text-[10px] font-black uppercase tracking-widest text-teal-400 border-b border-neutral-800 pb-1.5 mb-2 flex items-center justify-between">
                <span className="flex items-center gap-1">
                  <Bell size={12} /> Renewal & Lock-in Summary
                </span>
              ...
              <div className="space-y-1.5 text-[11px]">
                <div className="flex items-center justify-between py-1 px-1.5 bg-neutral-800/80 border border-neutral-700/60">
                  <span className="flex items-center gap-1.5 text-neutral-200">
                    <FileText size={12} className="text-teal-400" /> Agreement Renewals:
                  </span>
                  <span className={`font-bold font-mono ${summary.agreementCount > 0 ? 'text-amber-400' : 'text-neutral-400'}`}>
                    {summary.agreementCount} (within 60d)
                  </span>
                </div>

                <div className="flex items-center justify-between py-1 px-1.5 bg-neutral-800/80 border border-neutral-700/60">
                  <span className="flex items-center gap-1.5 text-neutral-200">
                    <Lock size={12} className="text-amber-400" /> Lock-In Expirations:
                  </span>
                  <span className={`font-bold font-mono ${summary.lockinCount > 0 ? 'text-red-400' : 'text-neutral-400'}`}>
                    {summary.lockinCount} (within 15d)
                  </span>
                </div>
              </div>

              <div className="mt-2 pt-1 border-t border-neutral-800 text-[9px] text-neutral-400 text-center italic">
                Click bell icon to open detailed section views
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-3 w-[380px] sm:w-[460px] bg-white border border-neutral-300 shadow-2xl z-50 overflow-hidden text-xs"
          >
            {/* Header */}
            <div className="p-4 bg-[#006064] text-white border-b border-teal-800">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-teal-100 font-light mt-0.5">
                      Client agreement & lock-in period tracker
                    </p>
                  </div>
                </div>
              ...
              {/* Section Tabs */}
              <div className="grid grid-cols-2 gap-2 bg-teal-950/60 p-1 border border-teal-700/60">
                <button
                  type="button"
                  onClick={() => setActiveTab('AGREEMENT')}
                  className={`py-2 px-2 text-[10px] font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                    activeTab === 'AGREEMENT'
                ...
                <div className="p-10 text-center space-y-3">
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-[#1B1C1C]">
                      {activeTab === 'AGREEMENT' ? 'No Agreements Ending Soon!' : 'No Lock-ins Ending Soon!'}
                    </h4>
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        >
                          {item.type === 'LOCK_IN' ? (
                            item.statusTag === 'EXPIRED'
                              ? `LOCK-IN EXPIRED (${Math.abs(item.daysRemaining)}d ago)`
            ...
            {totalCount > 0 && (
              <div className="p-3 bg-[#F8F9FA] border-t border-neutral-200 text-center">
                ...
                >
                  Manage All Records in Client Master <ChevronRight size={14} />
                </button>
              </div>
            )}
          </motion.div>
